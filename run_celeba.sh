...
CUDA_VISIBLE_DEVICES=$1 python main_linear.py \
  ...
  --ckpt '/C/eduardo/projects/SupContrast/save/SupCon/celeba_models/SimCLR_celeba_resnet50_lr_0.5_decay_0.0001_bsz_128_temp_0.5_trial_0_cosine/ckpt_epoch_900.pth' | tee save/celeba-linear-simclr-pre-900ep$(date +%y-%m-%d-%H-%M).txt


# # CE trainig
# CUDA_VISIBLE_DEVICES=$1 python main_ce.py --batch_size 128 \
#    --learning_rate 0.8  \
#    --cosine --syncBN \
#    --dataset 'celeba' \
#    --data_folder '/A/ngavenski/git/adversarial-bias/' \
#    --mean '(0.5071, 0.4867, 0.4408)' --std '(0.2675, 0.2565, 0.2761)' | tee save/ce-celeba-aug-$(date +%y-%m-%d-%H-%M).txt
